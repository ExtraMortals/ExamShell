...
import sentry_sdk
sentry_sdk.init("https://7580b0ddf85c48709587717ec3c1069e@sentry.42.fr/14")
...
def ensure_git(session):
    ...
    if not os.path.isfile("{}/private".format(EXAM_BASEDIR)):
    	try:
            with open("{}/private".format(EXAM_BASEDIR), "a") as f:
       	        f.write(session.ssh_private)
                f.close()
            os.chmod("{}/private".format(EXAM_BASEDIR), stat.S_IREAD)
        except Exception as e:
            print("ERROR {} when adding SSH keys".format(e))
            raise e

    print "Git repository is not cloned yet. Cloning..."
    try:
	collection_cmd = ['GIT_SSH_COMMAND=\'ssh -i {}/private\' git clone {} {}'.format(EXAM_BASEDIR, session.git_url, GIT_DIR) ]
        out = subprocess.check_output(collection_cmd, stderr=subprocess.STDOUT, shell=True)
    ...
    try:
	ssh_cmd = ['cd {}/rendu && git config --local core.sshCommand \'ssh -i {}/private\''.format(EXAM_BASEDIR, EXAM_BASEDIR) ]
        out = subprocess.check_output(ssh_cmd, stderr=subprocess.STDOUT, shell=True)
    except subprocess.CalledProcessError as e:
        print e.output
        fatal(("Could not set the shh git env variable from {0} (Return code"
              " {1}).").format(session.git_url, e.returncode), True)
...
class ExamShell(cmd.Cmd):
    ...
    def print_usage(self, session):
        self.no_next = session['current_project']['no_next']
        self.has_trace = session['current_project']['has_trace']
        print
    ...
    {1} to do and it will potentially bring you less points on your grade ...
    So be sure of yourself before you launch this command !""".format(colored("grademe", "green"), "the same assignment" if self.no_next else "another assignment of the same level")
    ...
    def help_grademe(self):
        print ("Asks the server to grade your current assignment. If you have"
               ...
               " and it will potentially bring you less points on your"
               " grade ... So\nbe sure of yourself before you launch this"
               " command !".format("the same assignment" if self.no_next else "an assignment of the same level"))
    ...
    def do_grademe(self, line):
        ...
        print ("If your assignment is wrong, you will have {} but with less potential points to earn !".format("the same assignment\n" if self.no_next else "another assignment of the same level\n"))
        print
        try:
        ...
        try:
            ...
            sess = get_session()
            if sess['current_project']['incremental_time']:
                last = datetime.datetime.strptime(sess['date_last_grading'], "%Y-%m-%dT%H:%M:%S.%fZ")
                nb_try = len([a for a in sess.assignments if a.level == sess.current_assignment.level])
                delay = fib(nb_try + 2)
                until = last + datetime.timedelta(minutes = delay)
                good_to_go = until - datetime.datetime.utcnow()
                s = good_to_go.total_seconds()
                hours, remainder = divmod(s, 3600)
                minutes, seconds = divmod(remainder, 60)
                good_to_go = "{:02}h{:02}m{:02}s".format(int(hours), int(minutes), int(seconds))
                print ("{}: You must wait at least {} until next grading request, so take your time to make more tests and be sure you will succeed next try!".format(colored("ERROR", "red"), colored(good_to_go, "yellow")))
            else:
                print ("{0}: You must wait at least {1} minutes between grading"
                   " requests").format(colored("ERROR", "red"), colored(2, "yellow"))
            return False
        ...
        try:
            ...
            if sess['current_project']['incremental_time']:
                ...
                minutes, seconds = divmod(remainder, 60)
                good_to_go = "{:02}h{:02}m{:02}s".format(int(hours), int(minutes), int(seconds))
                print ("{}: You must wait at least {} until next grading request, so take your time to make more tests and be sure you will succeed next try!".format(
                    colored("ERROR", "red"),
                    colored(good_to_go, "yellow")
                    )
                )
            else:
                print ("{0}: You must wait at least {1} minutes between grading"
...
if __name__ == "__main__":
    try:
        ...
        child = subprocess.Popen(['python', path + '/irl_checker.py'])
        version = get_version()
